...
class shootDiscSystem implements ISystem {
  ...
  update(dt: number): void {
    if (this.frisbee.isFired) {
      ...
      if (
        this.frisbee.body.velocity.almostEquals(new CANNON.Vec3(0, 0, 0), 0.25)
      ) {
        callout
          .getComponent(Transform)
          .position.set(
            this.frisbee.body.position.x,
            this.frisbee.body.position.y + 0.5,
            this.frisbee.body.position.z
          ) // Set callout position
        callout.addCallout()
      }
    } else {
